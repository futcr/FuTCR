...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.mask_classification:
            if use_text_embedding: # we donot use text embedding in this work(method1)
    ...
    def forward(self, x, mask_features, mask = None, distill_position = None, query_lib = None):
        ...
        #futcr_author1 ===== FEATURE EXTRACTION FOR CONTRASTIVE LOSS =====
        if self.training and self.kd_type in ['hybrid', 'contrastive'] :
            # features_curr: Current task decoder features
            if self.task > 1 and query_lib and self.vq_number > 0:
                features_curr_raw = tgt_undetach[:, :self.num_queries, :]
            else:
                features_curr_raw = tgt_undetach
            features_curr = self.flatten_features(features_curr_raw)
            
            # features_prev: Previous task features
            if distill_tgt_undetach is not None:
                features_prev = self.flatten_features(distill_tgt_undetach)
            else:
                features_prev = None
            
            # prototypes_prev: Old class prototypes
            if (distill_position is not None and distill_info is not None and 
                'pred_logits' in distill_info and 
                features_prev is not None):  # ← FIXED: Added check
                
                old_logits = distill_info['pred_logits']          # [B, Q, C_old]
                old_class_preds = old_logits.argmax(dim=-1)       # [B, Q]
                old_class_preds_flat = old_class_preds.reshape(-1) # [B*Q]
                
                C_old = old_logits.shape[-1]
                prototypes_prev = []
                for c in range(C_old):
                    mask = (old_class_preds_flat == c)
                    if mask.any():
                        prototype = features_prev[mask].mean(dim=0)  # Average features for class c
                    else:
                        prototype = torch.zeros(
                            features_prev.shape[-1], 
                            device=features_prev.device  
                        )
                    prototypes_prev.append(prototype)
                
                prototypes_prev = torch.stack(prototypes_prev)  # [C_old, D] 
            else:
                prototypes_prev = None
        else:
            features_curr = None
            features_prev = None
            prototypes_prev = None
        ...
        if not self.training:
            ...
            out = {
                ...
                #futcr_author1
                # ===== NEW: Features for contrastive loss =====
                'features_curr': features_curr, #[B*Q, D]
                'features_prev': features_prev, # [B*Q, D]
                'prototypes_prev': prototypes_prev, # [C_old, D]
            }
            
            
            #futcr_author1
            # if features_curr and features_prev and prototypes_prev:
            #     to_print ={
            #         'features_curr': features_curr.shape, #[B*Q, D]
            #         'features_prev': features_prev.shape, # [B*Q, D]
            #         'prototypes_prev': prototypes_prev.shape, # [C_old, D]
            #     }
            
            #     print(f"Training, TRANSF DEC OUT: {to_print}")
    ...
    def flatten_features(feat):
        """Flatten [batch, num_q, D] to [B*num_q, D]"""
        if feat is None:
            return None
        if feat.dim() == 3:
            batch_size, num_queries, feature_dim = feat.shape
            return feat.reshape(batch_size * num_queries, feature_dim)
        elif feat.dim() == 2:
            return feat
        else:
            return feat