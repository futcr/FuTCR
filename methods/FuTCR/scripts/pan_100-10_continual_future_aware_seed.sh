...
# Usage: bash pan_100-10_continual.sh <img_overlap_pct> <cls_overlap_pct> <base_cls> <inc_cls>
...
AUX_CLS_ENABLE=${12:-False}
AUX_CLS_NUM_CLUSTERS_MULTIPLIER=${13:-2} #set to 2 instead of 4 to match 100-5 setting
EXP_SEED=${14:-2}
KD_DECODER=${15:-True}
...
if [[ "$FUTURE_AWARE_ENABLE" == "True" ]]; then
    ...
    if [[ "$KD_DECODER" == "True" ]]; then
        extra_args+=(CONT.KD_DECODER True)
        OUTPUT_FLAG+=_KDD_${KD_DECODER}
    else
        OUTPUT_FLAG+=_KDD_${KD_DECODER} #just to indicate its false clearly
    fi
...
RESULT_DIR="./output/${TASKNAME}${OUTPUT_FLAG}_100-10/${OVERLAP_DIR}/${SCENARIO}"
...
if [ "$IMG_OV_PCT" -eq 0 ]; then
    STEP_IMG_COUNTS=(850 1146 1377 2142 1421)
else
    STEP_IMG_COUNTS=(854 1140 1372 2138 1419)
fi
