...
# Usage: bash pan_100-50_continual.sh <img_overlap_pct> <cls_overlap_pct> <base_cls> <inc_cls>
...
if [[ "$FUTURE_AWARE_ENABLE" == "True" ]]; then
    ...
    if [[ "$KD_DECODER" == "True" ]]; then
        ...
        extra_args+=(CONT.KD_DECODER False)
...
RESULT_DIR="./output/${TASKNAME}${OUTPUT_FLAG}_${SCENARIO}/${OVERLAP_DIR}/${SCENARIO}"
...
if [ "$IMG_OV_PCT" -eq 0 ]; then
    STEP_IMG_COUNTS=(4723)
else
    STEP_IMG_COUNTS=(4659)
fi
