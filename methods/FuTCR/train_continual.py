...
# Add standard_framework directory to path
#futcr_author1
...
from split_paths_resolver import get_standardized_predefined_split
...
def main(args):
    ...
    # futcr_author1 Get overlap % from args or config (default to 100 and 75)
    img_overlap =  getattr(args, 'img_overlap', None)
    cls_overlap =  getattr(args, 'cls_overlap', None)

    # panoptic segmentation
    if cfg.MODEL.MASK_FORMER.TEST.SEMANTIC_ON == True:
        #futcr_author1
        if img_overlap is not None and cls_overlap is not None:
            predefined_split = get_standardized_predefined_split(cfg, img_overlap, cls_overlap)
            print(f"USING STANDARDISED SPLITS WITH img_ov {img_overlap}, cls_ov {cls_overlap}")
            print(f"Task {cfg.CONT.TASK}: {cfg.CONT.BASE_CLS}-{cfg.CONT.INC_CLS}")
    ...
    #futcr_author1
    # return print(f"FUTURE AWARE PARAMS: {cfg.CONT.FUTURE_AWARE}" )
...
if __name__ == "__main__":
    ...
    #============OVERLAP EXPERIMENT=======================
    parser.add_argument("--img-overlap", default="100", 
                       help="Image Overlap  for continual splits (e.g., 100")
    parser.add_argument("--cls-overlap", default="75", 
                       help="Class Overlap  for continual splits (e.g., 75)")
    #============OVERLAP EXPERIMENT=======================
